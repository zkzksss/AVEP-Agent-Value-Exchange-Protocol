---
name: query-curves-by-creator
description: Query all BondingCurve addresses created by a given creator using BondingCurveFactory.getCurvesByCreator (curvesByCreator). Use when the user asks for curves by creator or list of curves created by an address.
---

# Query Curves by Creator

查询 `BondingCurveFactory.curvesByCreator`：根据创建者地址返回其创建的所有 curve 代理地址。
...
- `CREATOR`（必填）：要查询的创建者地址

## Contract

- `getCurvesByCreator(address creator) view returns (address[])`

## Script / Run Command

使用现有脚本 `scripts/utils/queryCurvesByCreator.ts`，仅传 `CREATOR`：

```bash
CREATOR="0xCreatorAddress" npx -y -p ts-node@10.9.2 -p ethers@6.15.0 -p zksync-ethers@6.0.0 -p dotenv@16.4.7 ts-node scripts/utils/queryCurvesByCreator.ts
```

## Output Expectations

- 输出 `creator:` 与 `curves:`（地址数组）；无创建记录时 `curves` 为空数组。
